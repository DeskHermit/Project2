...
if (!array_contains(no_rooms, room) && !array_contains(table_rooms, room)) {
    ...
    if (!instance_exists(obj_pause_sml)) {
        var _ins = instance_create_depth(_tx, _ty, -10000, obj_pause_sml);
                    ...
                    if (array_contains(table_rooms, room)) {
                        ...
                        if (item == obj_battery_3D) {
						    if (instance_exists(_wiz)) {
						        var _battery_missing = 100 - _wiz.battery;
								var _battery_refill = 100;
								var _battery_overfill = max(_battery_refill - _battery_missing, 0);
								var _battery_bonus = floor(_battery_overfill) * 10;
